...
import static kr.ac.inha.stress_sensor.services.CustomSensorsService.DATA_SRC_STATIONARY_DUR;
        ...
        if (event.sensor.getType() == Sensor.TYPE_LINEAR_ACCELERATION) {
            ...
            if (diff > 0.5) // 0.5 is a threshold, you can test it and change it
            {
                if (curTimestamp - stationaryDetectEndTime > 5000) {
                    ...
                    if (duration > 0) {
                        String value = (stationaryDetectStartTime - 5000) + " " + stationaryDetectEndTime + " " + duration + " " + Tools.getEMAOrderFromRangeBeforeEMA(System.currentTimeMillis());
