...
public class Tools {
    ...
    public static void checkAndSendUsageAccessStats(Context con) throws IOException {
        ...
        for (UsageStats usageStats : stats) {
            ...
            if (usageStats.getLastTimeUsed() > cal.getTimeInMillis()) //if last time used is more than January 1, 2019 then take the stats (to eliminate extra app usages)
            {
                ...
                if (sb.length() == 0) {
                    sb.append(String.format(
                            ...
                            Tools.getEMAOrderFromRangeBeforeEMA(usageStats.getLastTimeUsed())
                    ));
                } else {
                    sb.append(String.format(
                            Locale.getDefault(),
